...
class LiveMatch:
    ...
    def build_scoreboard(self, include_stats=True) -> dict:
        ...
        elif meta.get("loadouts"):
            weapons_by_puuid = meta["loadouts"]

        pmap = self.party_map(puuids, presences)
        if not pmap:
            own_party = [m["puuid"] for m in self.party_members(presences)
                         if m.get("puuid") in puuids]
            if len(own_party) > 1:
                pmap = {"local-party": own_party}
        party_lookup = {}
        parties_out = []
        ...
        for p in raw_players:
            ...
            players.append(assemble_player(
                ...
                economy={"loadoutValue": loadout_value} if loadout_value is not None else None,
